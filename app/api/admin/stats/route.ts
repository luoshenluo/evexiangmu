...
import { getAllUsers, listAdminLogs, getListings, getBuyOrders, getChatStats } from '@/lib/server-store'
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (action === 'overview') {
      ...
      const [users, listings, buyOrders, logsResult, chatStats] = await Promise.all([
        getAllUsers(),
        getListings(),
        getBuyOrders(),
      ...
      const onlineUsers = users.filter(u => {
        const la = (u as any).lastActiveAt || u.lastLogin || 0
        return la >= fiveMinutesAgo
      }).length
      const todayNewUsers = users.filter(u => u.createdAt >= todayStartTs).length
      const todayMessages = (chatStats && typeof chatStats.todayCount === 'number') ? chatStats.todayCount : 0
      ...
      const inventoryFlowerValue = allInventoryItems.filter(i => i.type === 'flower' && i.quantity > 0).reduce((s, i) => s + ((i as any).price || 10) * i.quantity, 0)
      ...
      const avgGrowth = allPlots.filter(p => p.flower).length > 0 ? Math.round(allPlots.filter(p => p.flower).reduce((s, p) => s + (p.flower?.growthProgress || 0), 0) / allPlots.filter(p => p.flower).length) : 0
      ...
      for (const u of users) {
        for (const plot of u.plots || []) {
          ...
          if (fl && fl.typeId) {
            const key = fl.typeId
            if (!topFlowersMap[key]) topFlowersMap[key] = { id: key, name: fl.name || key, emoji: fl.emoji || '🌸', count: 0 }
    ...
    if (action === 'export_users') {
      ...
      return new Response('\uFEFF' + lines.join('\n'), { status: 200, headers: { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': `attachment; filename="users_${Math.floor(now/1000)}.csv"` } })
    }
    ...
    if (action === 'export_logs') {
      ...
      return new Response('\uFEFF' + lines.join('\n'), { status: 200, headers: { 'content-type': 'text/csv; charset=utf-8', 'content-disposition': `attachment; filename="admin_logs_${Math.floor(now/1000)}.csv"` } })
    }
