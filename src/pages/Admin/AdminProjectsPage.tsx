...
import {
  Plus, Search, Edit3, Trash2, Package, Loader2, Ship, Shield, Swords,
  Anchor, Crosshair, Factory, ChevronRight, ArrowLeft,
} from 'lucide-react';
...
export default function AdminProjectsPage() {
  ...
  useEffect(() => {
    if (categoryNames.length > 0 && !categoryNames.includes(activeCat)) setActiveCat(categoryNames[0]);
  }, [categoryNames, activeCat]);

  const currentCatProjects = useMemo(() => {
    let list = categories.get(activeCat) ?? [];
    if (keyword.trim()) { const kw = keyword.trim().toLowerCase(); list = list.filter((p) => p.name.toLowerCase().includes(kw)); }
    return list;
  }, [categories, activeCat, keyword]);
    ...
    return (
      <div className="p-4 md:p-6">
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-lg font-semibold text-white">制造项目管理</h2><p className="mt-0.5 text-sm text-[#A0A0A0]">选择大类管理项目，所有用户可见</p></div>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-[#7C3AED]" /><span className="ml-2 text-sm text-[#A0A0A0]">加载中...</span></div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {categoryNames.map((cat) => {
              ...
              return (
                <button key={cat} onClick={() => { setActiveCat(cat); setView('list'); }}
                  className="flex flex-col items-center gap-2 rounded-xl border border-[#3A3A3A] bg-[#2C2C2C] p-5 transition-all hover:border-[#7C3AED]/60 active:scale-[0.97]">
                  <div className="flex h-14 w-14 items-center justify-center rounded-xl bg-[#1E1E1E] border border-[#3A3A3A]">
  ...
  return (
    <div className="p-4 md:p-6">
      <div className="mb-4 flex items-center gap-3">
        <button onClick={() => setView('categories')} className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#2C2C2C] border border-[#3A3A3A] text-white transition-colors hover:border-[#555555]"><ArrowLeft className="h-4 w-4" /></button>
        ...
        <button onClick={() => navigate('/admin/projects/new')} className="flex items-center justify-center gap-1.5 rounded-lg bg-[#7C3AED] px-3 py-2 text-sm font-medium text-white shadow-[0_2px_8px_rgba(124_58_237_0.3)] transition-all hover:bg-[#6D28D9] active:scale-[0.98]"><Plus className="h-4 w-4" />新增</button>
      </div>
      <div className="mb-4 relative w-full md:max-w-md">
        ...
        <input type="text" value={keyword} onChange={(e) => setKeyword(e.target.value)} placeholder={`在${activeCat}中搜索...`} className="w-full rounded-md border border-[#444444] bg-[#2C2C2C] pl-9 pr-3 py-2 text-sm text-white placeholder-[#666666] outline-none transition-all focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/30" />
      </div>
        ...
        {loading ? <div className="flex items-center justify-center py-12"><Loader2 className="h-6 w-6 animate-spin text-[#7C3AED]" /><span className="ml-2 text-sm text-[#A0A0A0]">加载中...</span></div> : (
          <div className="w-full overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {currentCatProjects.length === 0 ? <tr><td colSpan={6} className="px-4 py-12 text-center"><div className="flex flex-col items-center gap-2 text-[#888888]"><Package className="h-8 w-8 opacity-50" /><span className="text-sm">暂无匹配的项目</span></div></td></tr> : currentCatProjects.map((project) => (
