...
function loadFromStorage<T>(key: string, fallback: T): T {
  try { const raw = lsGetItem(key); if (raw) { return JSON.parse(raw) as T; } } catch {}
  return fallback;
}
...
export default function HomePage() {
  ...
  useEffect(() => { lsSetItem(STORAGE_KEYS.activeTab, JSON.stringify(activeTab)); }, [activeTab]);
  useEffect(() => { lsSetItem(STORAGE_KEYS.minerals, JSON.stringify(minerals)); }, [minerals]);
  useEffect(() => { lsSetItem(STORAGE_KEYS.shipMaterials, JSON.stringify(shipMaterials)); }, [shipMaterials]);
  useEffect(() => { lsSetItem(STORAGE_KEYS.buildMaterials, JSON.stringify(buildMaterials)); }, [buildMaterials]);
  useEffect(() => { lsSetItem(STORAGE_KEYS.calcParams, JSON.stringify(calcParams)); }, [calcParams]);
  ...
  useEffect(() => {
    const syncAdminPrices = async () => {
      try {
        ...
        if (mineralPrices.length > 0) { setMinerals((prev) => prev.map((item) => { const c = mineralPrices.find((p) => p.name === item.name); return c ? { ...item, price: c.price } : item; })); }
        if (shipPrices.length > 0) { setShipMaterials((prev) => prev.map((item) => { const c = shipPrices.find((p) => p.name === item.name); return c ? { ...item, price: c.price } : item; })); }
        if (buildPrices.length > 0) { setBuildMaterials((prev) => prev.map((item) => { const c = buildPrices.find((p) => p.name === item.name); return c ? { ...item, price: c.price } : item; })); }
      } catch (err) { console.warn('Failed to sync admin prices:', err); }
    };
    syncAdminPrices();
  }, []);
  ...
  const renderContent = () => {
    ...
    return <div className="px-4 md:px-6 lg:px-8 py-4 md:py-6">{content}</div>;
  };
