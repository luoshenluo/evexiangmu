...
export default function MaterialInputSection({ title, subtitle, materials, onChange }: MaterialInputSectionProps) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-40 md:pb-4">
        <div className="space-y-2">
          {materials.map((item, index) => {
            ...
            return (
              <div key={item.name} className="rounded-lg bg-[#2C2C2C] border border-[#3A3A3A] px-3 py-2.5 transition-all hover:border-[#555555]">
                <div className="grid grid-cols-[1fr_90px_90px] gap-2 items-center">
                  <div className="text-sm font-medium text-white truncate">{item.name}</div>
                  <NumberInput value={item.price} onChange={(num) => handlePriceChange(index, num)} step="1" placeholder="0"
                  ...
                  <NumberInput value={item.quantity} onChange={(num) => handleQuantityChange(index, num)} step="1" placeholder="0"
                    className="w-full rounded-md border border-[#444444] bg-[#1E1E1E] px-2 py-1.5 text-right text-sm text-white placeholder-[#666666] outline-none transition-all focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/30 hover:border-[#666666]" />
                </div>
                <div className="mt-1.5 flex items-center justify-between text-[11px]">
                  <span className="text-[#888888]">单价：ISK/个 · 数量：个</span>
                  <span className="font-semibold text-[#A78BFA] tabular-nums">{formatNumber(itemTotal)} 亿ISK</span>
                </div>
              </div>
            );
          })}
      ...
      <div className="fixed bottom-16 md:static md:mt-4 md:border md:rounded-xl left-0 right-0 md:left-auto md:right-auto border-t md:border-t-0 border-[#3A3A3A] bg-[#1E1E1E]/95 md:bg-[#2C2C2C] backdrop-blur-md px-4 py-3">
        <div className="mx-auto flex max-w-md gap-2">
          <button onClick={handleClearPrices} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-[#444444] bg-[#2C2C2C] px-2 py-2.5 text-xs font-medium text-white transition-all hover:border-[#666666] hover:bg-[#363636] active:scale-[0.98]">
            <Eraser className="h-3.5 w-3.5" />清空单价
          </button>
          <button onClick={handleClearQuantities} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-[#444444] bg-[#2C2C2C] px-2 py-2.5 text-xs font-medium text-white transition-all hover:border-[#666666] hover:bg-[#363636] active:scale-[0.98]">
            <Eraser className="h-3.5 w-3.5" />清空数量
          </button>
          <button onClick={handleSummary} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-[#7C3AED] px-2 py-2.5 text-xs font-medium text-white shadow-[0_2px_8px_rgba(124_58_237_0.3)] transition-all hover:bg-[#6D28D9] active:scale-[0.98]">
            <CalcIcon className="h-3.5 w-3.5" />汇总总价
          </button>
        </div>
      </div>
    </div>
