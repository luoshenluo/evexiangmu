  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pb-2">
        <div className="hidden sm:grid sm:grid-cols-[1fr_100px_100px] gap-2 text-[11px] font-medium text-[#888888]">
          <div>材料名称</div>
        ...
        <div className="space-y-2">
          {materials.map((item, index) => {
            ...
            return (
              ...
              >
                {/* 材料名称 */}
                <div className="text-sm font-medium text-white truncate mb-2 xs:mb-0">{item.name}</div>
                {/* 输入区域：移动端垂直堆叠，xs+ 水平排列 */}
                <div className="grid grid-cols-1 xs:grid-cols-[1fr_1fr] gap-1.5">
                  <div className="flex xs:flex-col items-center xs:items-stretch gap-1.5">
                    <span className="xs:hidden text-[10px] text-[#888] shrink-0 w-10">单价</span>
                    <NumberInput
                      value={item.price}
                      onChange={(num) => handlePriceChange(index, num)}
                      step="1"
                      placeholder="0"
                      className="flex-1 w-full rounded-md border border-[#444444] bg-[#1E1E1E] px-2 py-1.5 text-right text-sm text-white placeholder-[#666666] outline-none transition-all focus:border-[#7C3AED] focus:ring-2 focus:ring-[#7C3AED]/30 hover:border-[#666666]"
                    />
                  </div>
                  <div className="flex xs:flex-col items-center xs:items-stretch gap-1.5">
                    <span className="xs:hidden text-[10px] text-[#888] shrink-0 w-10">数量</span>
                    <NumberInput
                      value={item.quantity}
                ...
                <div className="mt-1.5 flex items-center justify-between text-[11px]">
                  <span className="text-[#888888]">ISK/个 · 个</span>
